...
//    System.out.println(GraphBuilder.drawGraph(architecture));
    ...
    for (Component sourceComponent : architecture.getComponents()) {
      for (Entry<Class, ArrayList<Class>> entry : sourceComponent.getConnection().entrySet()) {
        if (sourceComponent.getUses().get(entry.getKey()) != null) {
          System.out.println(
              sourceComponent.getComponentName() + " - " +
                  entry.getKey() + " -> " +
                  entry.getValue().toString() + " - " +
                  sourceComponent.getUses().get(entry.getKey()).getComponentName()
          );
        }
      }
    }
